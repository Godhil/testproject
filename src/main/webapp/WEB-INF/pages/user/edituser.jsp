...
<div id="container">
    ...
    <form id="userForm">
        ...
        <input type="text" name="id" id="idInput" required="true" readonly/>
        <br/>
...
//        $('#value').text(userId);
        ...
        $.get('${pageContext.request.contextPath}/api/user/' + userId, function (user) {
                $('#idInput').val(user.id);
                $('#loginInput').val(user.login);
                $('#passInput').val(user.password);
                $('#surnameInput').val(user.surname);
                $('#firstNameInput').val(user.firstName);
                $('#secondNameInput').val(user.secondName);
                $('#roleInput').val(user.role);
                $('#municipalityInput').val(user.municipality.id);
            });
        ...
        $('#userForm').submit(function (e) {
            $.post('${pageContext.request.contextPath}/api/user/edit', $(this).serialize(), function (response) {
                $('#formResponse').text('Сохранено')
            });
            e.preventDefault();
        });
